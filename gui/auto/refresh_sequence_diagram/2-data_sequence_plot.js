...
const blockUniq = [...new Set([...results.sequence_event_records.map(row => row.networkElement1), ...results.sequence_event_records.map(row => row.networkElement2)])];
...
const rows = results.sequence_event_records.map(row => ({
    date: row.timestamp,
