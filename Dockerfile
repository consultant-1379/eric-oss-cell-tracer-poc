...
ARG USER_NAME="eric-oss-cell-tracer-poc"
RUN echo "$USER_ID:x:$USER_ID:0:An Identity for $USER_NAME:/nonexistent:/bin/false" >>/etc/passwd

ARG APP_MODULE=eric-oss-cell-tracer-poc-app
ARG APP_VERSION
COPY gui/ /gui/
ADD ${APP_MODULE}/target/${APP_MODULE}-${APP_VERSION}.jar ${APP_MODULE}.jar
COPY ${APP_MODULE}/src/main/resources/jmx/* /jmx/
RUN chmod 600 /jmx/jmxremote.password
RUN chown $USER_ID /jmx/jmxremote.password

COPY scripts/generate_jks.sh /usr/local/bin/
RUN chmod +x /usr/local/bin/generate_jks.sh
ENTRYPOINT ["./usr/local/bin/generate_jks.sh"]
...
CMD ["/bin/sh", "-c", "java ${JAVA_OPTS} -Dcom.sun.management.jmxremote=true \
    ...
    -jar eric-oss-cell-tracer-poc-app.jar"]
...
LABEL \
    org.opencontainers.image.title=eric-oss-cell-tracer-poc-jsb \
