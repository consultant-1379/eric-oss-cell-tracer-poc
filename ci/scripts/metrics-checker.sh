...
checkPomXML(){
    ...
    grep -B2 -A7 "<artifactId>micrometer-core</artifactId>" ./eric-oss-cell-tracer-poc-app/pom.xml | sed 's/^[ \t]*//;s/[ \t]*$//' > .bob/var.scrapedLine || true;
...
checkCoreApplicationJAVA(){
    ...
    find ./eric-oss-cell-tracer-poc-app/src/main/java -regex '.*Application.java' -exec grep -o 'public WebClient webClient(final WebClient.Builder webClientBuilder,' {} \; > .bob/var.scrapedLine || true;
    find ./eric-oss-cell-tracer-poc-app/src/main/java -regex '.*Application.java' -exec grep -o 'return webClientBuilder' {} \; >> .bob/var.scrapedLine || true;
